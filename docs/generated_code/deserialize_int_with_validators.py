# marshmallow.fields.Integer with validators
def _jit_deserialize_SerializeIntegerWithValidatorsSchema_1(
                            self,
                            data: (
        ...
        try:
            if value is missing:
                ...
                if value is True or value is False:
                    raise field_2.make_error("invalid", input=value)
                try:
                    value = int(value)
                except (TypeError, ValueError) as error:
                    raise field_2.make_error("invalid", input=value) from error
                except OverflowError as error:
                    raise field_2.make_error("too_large", input=value) from error
                validation_errors_7 = []
                validation_kwargs_8 = {}
                try:
